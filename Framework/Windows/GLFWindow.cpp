...
namespace Vultana
{
    ...
    void GLFWindow::SetSize(Vector2 size)
    {
        glfwSetWindowSize(this->mHwnd, (int)size.x, (int)size.y);
    }

    Vector2 GLFWindow::GetSize() const
    {
    ...
    void GLFWindow::SetPosition(Vector2 position)
    {
        glfwSetWindowPos(this->mHwnd, (int)position.x, (int)position.y);
    }

    Vector2 GLFWindow::GetPosition() const
    {
        int x = 0, y = 0;
    ...
    Vector2 GLFWindow::GetCursorPosition() const
    {
        double x = 0, y = 0;
    ...
    void GLFWindow::SetCursorPosition(Vector2 position)
    {
        glfwSetCursorPos(this->mHwnd, (int)position.x, (int)position.y);
    }

    CursorMode GLFWindow::GetCursorMode() const
    {
        return (CursorMode)glfwGetInputMode(this->mHwnd, GLFW_CURSOR);
    }

    void GLFWindow::SetCursorMode(CursorMode mode)
    {
        glfwSetInputMode(this->mHwnd, GLFW_CURSOR, (int)mode);
    }

    bool GLFWindow::IsKeyPressed(KeyCode key) const
    {
        return glfwGetKey(this->mHwnd, (int)key) == GLFW_PRESS;
    }

    bool GLFWindow::IsKeyReleased(KeyCode key) const
    {
        return glfwGetKey(this->mHwnd, (int)key) == GLFW_RELEASE;
    }

    bool GLFWindow::IsMousePressed(MouseButton button) const
    {
        return glfwGetMouseButton(this->mHwnd, (int)button) == GLFW_PRESS;
    }

    bool GLFWindow::IsMouseReleased(MouseButton button) const
    {
    ...
    void GLFWindow::OnResize(std::function<void(GLFWindow &, Vector2)> callback)
    {
        this->mOnResize = std::move(callback);
    }

    void GLFWindow::OnKeyChanged(std::function<void(GLFWindow &, KeyCode, bool)> callback)
    {
        this->mOnKeyChanged = std::move(callback);
    }

    void GLFWindow::OnMouseChanged(std::function<void(GLFWindow &, MouseButton, bool)> callback)
    {
